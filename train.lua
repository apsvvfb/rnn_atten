...
cmd:option('-feat_dim',1024,'size of the rnn in number of hidden nodes in each layer')
...
local wparams, grad_wparams = protos.atten:getParameters()
if string.len(opt.start_from) > 0 then
...
local function eval_split(split)
	...
	while true do
		...
		word_feat, img_feat, w_ques, w_img, mask = unpack(protos.atten:forward({data.questions, new_data_images}))
...
local function lossFun()
	...
	word_feat, img_feat, w_ques, w_img, mask = unpack(protos.atten:forward({data.questions, data.images}))
	-- forward the language model criterion
	local loss = protos.crit:forward(out_feat, data.answer)
	-----------------------------------------------------------------------------
	-- Backward pass
	...
	local dummy = protos.atten:backward({data.questions,data.images}, {d_conv_feat, d_w_ques, d_w_img, d_conv_img, d_ques_img})
	-----------------------------------------------------------------------------
	-- and lets get out!
	local stats = {}
	stats.dt = dt
